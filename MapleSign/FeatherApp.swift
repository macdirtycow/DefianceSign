...
struct FeatherApp: App {
	...
	private func _handleURL(_ url: URL) {
		if url.scheme == "maplesign" {
            ...
            if url.pathExtension == "maplesign" {
                UIAlertController.showAlertWithOk(title: .localized("Error"), message: .localized("MapleSign certificate file (.maplesign) is now unsupported from v1.5.1, please refer to use .p12 and .mobileprovision instead."))
            }
		}
	}
...
class AppDelegate: NSObject, UIApplicationDelegate {
    ...
    private func _createPipeline() {
        ...
        let pipeline = ImagePipeline {
            ...
            let dataCache = try? DataCache(name: "net.maplesign.app.datacache") // disk cache
