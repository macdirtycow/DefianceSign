...
struct FeatherApp: App {
	init() {
		DefianceSignLocale.applyEnglish()
	}
	...
	private func _handleURL(_ url: URL) {
		if url.scheme == "defiancesign" {
            ...
            if url.pathExtension == "defiancesign" {
                UIAlertController.showAlertWithOk(title: .localized("Error"), message: .localized("DefianceSign certificate file (.defiancesign) is now unsupported from v1.5.1, please refer to use .p12 and .mobileprovision instead."))
            }
		}
	}
...
class AppDelegate: NSObject, UIApplicationDelegate {
    ...
    private func _createPipeline() {
        ...
        let pipeline = ImagePipeline {
            ...
            let dataCache = try? DataCache(name: "net.defiancesign.app.datacache") // disk cache
