...
struct FileRow: View {
            ...
            Group {
                if file.isDirectory {
                    ...
                    Image(systemName: "key")
                        .foregroundColor(.accentColor)
                } else if file.isDefianceSignFile {
    ...
    private func fileConfirmationDialogButtons() -> some View {
        ...
        if file.isDefianceSignFile {
            Button {
                UIAlertController.showAlertWithOk(title: "?", message: .localized("DefianceSign certificate file (.defiancesign) is now unsupported from v1.5.1, please refer to use .p12 and .mobileprovision instead."))
            } label: {
